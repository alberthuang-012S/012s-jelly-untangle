...
- 「復原」只回復上一次完成的 Jelly Move；「重來」回到本題初始配置。教學開始時以中央彈窗顯示「嘗試移動水母，讓紅線不再交叉。」，按「開始嘗試」後進行操作。
- 各難度從第 1 關開始解鎖，完成第 N 關後解鎖第 N+1 關；解鎖進度會儲存在瀏覽器 localStorage。

## Difficulty

- 每種難度各有 10 關，先選擇難度與關卡，再開始挑戰；通關後按「下一關」進入同難度的下一個固定關卡。
